...
package dev.ovis.futureplots.core.components.util;
...
public class Plot {
    ...
    public boolean addTrust(UUID playerId) {
        if(this.isTrusted(playerId)) return false;

        this.manager.getConnectedPlots(this).forEach(plot -> plot.addTrust0(playerId));
        return true;
    }
    ...
    private void removeTrusted0(UUID playerId) {
        this.trusted.remove(playerId);
    }
    ...
    public Object getFlagValue(String name) {
        if(this.flags.get(name) == null) {
            return FlagRegistry.getFlagByName(name).getDefaultValue();
        }
        return this.flags.get(name);
    }

    public void setFlagValue(String name, Object value) {
        this.manager.getConnectedPlots(this).forEach(plot -> {
            plot.setFlagValue0(name, value);
        });
    }

    private void setFlagValue0(String name, Object value) {
        this.flags.put(name, value);
    }

    public void removeFlag(String name) {
        this.manager.getConnectedPlots(this).forEach(plot -> {
            plot.removeFlag0(name);
        });
    }

    private void removeFlag0(String name) {
    ...
    public static Builder builder(PlotManager manager, PlotId id) {
    ...
    public static final class Builder {
        ...
        public Plot build() {
            ...
            if(this.trusted != null && !this.trusted.isEmpty()) plot.trusted.addAll(this.trusted);
            if(this.helpers != null && !this.helpers.isEmpty()) plot.helpers.addAll(this.helpers);
